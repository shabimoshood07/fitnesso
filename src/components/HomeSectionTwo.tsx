  ...
  return (
    <section className="bg-[#eef0f4] py-[75px] md:py-[100px] lg:py-[150px]">
      <div className="w-[97%] mx-auto max-w-[1450px] ">
        <h1 className="mb-[30px] max-w-[800px] text-[45px] md:text-[70px] lg:text-[90px] leading-[1.1em] font-[500]  ">
        ...
        <button className="btn !py-[20px] !min-w-[200px] !w-auto ">
          View all
        </button>

        <div className="pt-[50px] grid grid-cols-1 gap-x-[25px] gap-y-[25px] md:grid-cols-3 items-stretch justify-stretch border border-green-500">
          {data.map((dat, index) => {
            return (
              <div
                className="rounded-md overflow-hidden hover:-translate-y-1 hover:opacity-70 duration-500 group cursor-pointer flex justify-stretch flex-col items-stretch"
                key={index}
              >
                <div
                  className="w-full h-[250px] bg-center bg-no-repeat bg-cover"
                  style={{
                    backgroundImage: `url(${dat.image})`,
                  }}
                ></div>

                <div className="p-[20px] bg-white  flex flex-col justify-between ">
